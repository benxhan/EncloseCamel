...
let button_rect x y texture =
  Rectangle.create
    (float x)
    (float y)
    (float (Texture.width texture))
    (float (Texture.height texture))
...
let draw_button x y texture label =
  draw_texture_ex texture (Vector2.create (float x) (float y)) 0.0 1.0 Color.white;
  draw_rectangle_lines x y (Texture.width texture) (Texture.height texture) Color.black;
  let text_width = measure_text label 20 in
  draw_text
    label
    (x + (Texture.width texture - text_width) / 2)
    (y + (Texture.height texture - 20) / 2)
    20 Color.black
...
let draw_scene_title title =
  ...
  draw_text
    ...
    200 180 24 Color.darkgray

let find_menu_action buttons =
  match
    List.find_opt
      (fun (x, y, texture, _, _) -> button_clicked (button_rect x y texture))
      buttons
  with
