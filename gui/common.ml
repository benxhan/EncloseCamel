...
let compute_gui_tile_px ?(reserve_tip_footer = 0) board =
  ...
  let h_pad = 32 in
  let v_pad = 16 in
  let reserve_bottom = Render.info_panel_height + 80 + reserve_tip_footer in
      ...
      let lines_rev, last_line =
        List.fold_left
          (fun ((acc : string list), curr) raw_w ->
            let cand = if curr = "" then raw_w else curr ^ " " ^ raw_w in
...
let cell_at_mouse board x y =
  ...
  if y < 0 || y >= height then None else Some { r = y / ts; c = x / ts }
...
let draw_text_info ?(offset_x = 0) board status_message =
  ...
  draw_rectangle offset_x info_y walls_width walls_height
    (Color.create 0 0 0 128);
  draw_text walls_text
    (offset_x + 10 + ((walls_width - 20 - measure_text walls_text 20) / 2))
    (info_y + 2) 20 Color.white;

  let status_width = measure_text status_message 18 + 20 in
  let status_height = 20 in
  draw_rectangle offset_x (info_y + 26) status_width status_height
    (Color.create 0 0 0 128);
  draw_text status_message
    (offset_x + 10 + ((status_width - 20 - measure_text status_message 18) / 2))
    (info_y + 28) 18 Color.white
...
let draw_status_panel ?(offset_x = 0) board status_message =
  ...
  draw_rectangle offset_x panel_y width Render.info_panel_height
