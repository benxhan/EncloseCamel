...
let run_game_scene assets board =
  ...
  let back_button_y = board_pixel_height board + Render.info_panel_height + 10 in
  let button_width = int_of_float (float (Texture.width assets.back) *. scale) in
  let button_height = int_of_float (float (Texture.height assets.back) *. scale) in
  ...
  let rec loop board status_message =
    if window_should_close () then Quit
      ...
      draw_rectangle back_button_x back_button_y button_width button_height (Color.create 110 155 80 255);
      draw_rectangle_lines back_button_x back_button_y button_width button_height Color.black;
      let text_width = measure_text "Back" 20 in
      draw_text "Back" (back_button_x + (button_width - text_width) / 2) (back_button_y + (button_height - 20) / 2) 20 Color.white;
      end_drawing ();
